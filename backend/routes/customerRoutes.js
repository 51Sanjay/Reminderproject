...
router.post('/', async (req, res) => {
  try {
    const { name, email, phone, purchaseDate, reminderDate, note, preferredDelivery } = req.body;
...
router.put('/:id', async (req, res) => {
  try {
    const updateData = {};
    ['name', 'email', 'phone', 'note', 'preferredDelivery'].forEach(key => {
...
router.get('/calendar', async (req, res) => {
  try {
    const customers = await Customer.find({ reminderDate: { $ne: null } });
    const events = customers.map(c => ({
      id: c._id,
      title: `Reminder: ${c.name}`,
      start: c.reminderDate,
    }));
    res.json(events);
  } catch (err) {
    res.status(500).json({ error: err.message });
